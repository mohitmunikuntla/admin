...
Cypress.Commands.add(
    'createProject',
    (projectId = 'bf', name = 'My Project', defaultLanguage = 'en', defaultDevelopmentService = "rasa_development", defaultProductionService = "rasa_production", trainingService = "rasa_development",) => {
        const project = {
            _id: projectId,
            name,
            defaultLanguage,
        ...
        return cy.fixture('lite-policies.yaml').then(policies => cy.window().then(async ({ Meteor }) => {
            ...
            return cy.createNLUModelProgramatically(projectId, '', defaultLanguage, defaultDevelopmentService, defaultProductionService, trainingService);
        }));
    },
);
