...
import { ClaiService } from '../imports/api/claiservice';
...
import { ENVIRONMENT_OPTIONS } from '../imports/ui/components/constants.json';
...
Migrations.add({
    ...
    up: () => {
        try{
            ...
            const CLAIENVSPATH = privateSettings.CLAIENVSPATH;
    
            GlobalSettings.update(
                { _id: 'SETTINGS' },
                { $set: { 'settings.private.CLAIENVSPATH': CLAIENVSPATH } },
            );
        } catch(e){
            throw new Error(
                `Something went wrong during a migration, contact Clai for further help: ${e}`,
...
Migrations.add({
    ...
    up: async () => {
        try{
            Projects.find({defaultDevelopmentService: {$exists: false}})
            .fetch()
            .forEach((i) => {
                Projects.update(
                    {_id: i._id},
                    { $set : { defaultDevelopmentService: null }}
                );
            })
        } catch(e){
            throw new Error(
                `Something went wrong during a migration, contact Clai for further helps: ${e}`,
            );
        }
    }
});
...
Migrations.add({
    ...
    up: async () => {
        try{
            Projects.find({defaultProductionService: {$exists: false}})
            .fetch()
            .forEach((i) => {
                Projects.update(
                    {_id: i._id},
                    { $set : { defaultProductionService: null }}
                );
            })
        } catch(e){
            throw new Error(
...
Migrations.add({
    ...
    up: async () => {
        try{
            Projects.find({trainingService: {$exists: false}})
            .fetch()
            .forEach((i) => {
                Projects.update(
                    {_id: i._id},
                    { $set : { trainingService: null }}
                );
            })
        } catch(e){
            throw new Error(
...
Migrations.add({
    ...
    up: async () => {
        try{
            Projects.find({$and: [{deploymentEnvironments: []}, {defaultDevelopmentService: {$exists: false}}]})
            .fetch()
            .forEach((i) => {
                let service = ClaiService.findOne({projectId: i._id});
...
Migrations.add({
    ...
    up: async () => {
        try{
            Projects.find({$and: [{deploymentEnvironments: ['production']}, {defaultProductionService: {$exists: false}}, {defaultDevelopmentService: {$exists: false}}]})
            .fetch()
            .forEach((i) => {
                let service = ClaiService.findOne({projectId: i._id});
                if(service == undefined){
