...
if (Meteor.isServer) {
    ...
    Meteor.methods({
        ...
        async exportRasa(projectId, language, options) {
            ...
            // const claiservice = await Promise.all(
            //     envs.map(async (environment) => {
            //         const serv = await ClaiService.findOne(
            //             { projectId, ...getEnvQuery('environment', environment) },
            //             { fields: { claiservice: 1 } },
            //         );
            //         return {
            //             environment,
            //             claiservice: serv.claiservice,
            //         };
            //     }),
            // );
            ...
            let claiservice;
            var instance = {};
            const InstanceProject = await Projects.findOne({_id: projectId});
            ...
            //     );
            // });
            // claiservice.forEach(({ claiservice: claiservicePerEnv, environment }) => {
            //     rasaZip.addFile(
            //         claiservicePerEnv,
            //         `claiservice${envSuffix(environment, claiservice)}.yml`,
            //     );
            // });
