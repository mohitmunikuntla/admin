    ...
    Meteor.methods({
        ...
        async 'nlu.chitChatSetup'() {
            ...
            try {
                ...
                // const claiservice = safeLoad(Assets.getText('defaults/claiservice.yaml'));
                var claiservice;
                if(process.env.CLAIENVSPATH == undefined){
                    const fields = {
                ...
                const projectId = await Meteor.callWithPromise('project.insert', {
                    ...
                    namespace: 'bf-clai',
                    defaultLanguage: 'en',
                    defaultDevelopmentService: Object.keys(claiservice)[0] || '',
                    defaultProductionService: null,//Object.keys(claiservice)[1] || ''
                    trainingService: null,
                    languages: [],
                    deploymentEnvironments: [],
                });
                
                ClaiService.update({projectId: 'clai-defaultid', environment: 'development'}, {$set: {claiservice: Object.keys(claiservice)[0] || ''}});

                // ClaiService.update({projectId: 'clai-defaultid', environment: 'production'}, {$set: {claiservice: Object.keys(claiservice)[1] || ''}});
