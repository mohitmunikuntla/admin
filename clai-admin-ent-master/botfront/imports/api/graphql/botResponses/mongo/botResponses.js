...
export const deleteResponse = async (projectId, key) => {
    const response = typeof(BotResponses) != 'object' ? (await BotResponses.findOne({ projectId, key }).lean()) : [] ;
    if (!response) return;
    ...
    return typeof(Forms) != 'object' ? (BotResponses.findOneAndDelete({ _id: response._id }).lean()) : [] ; // eslint-disable-line consistent-return
};
...
export const deleteResponsesRemovedFromStories = async (removedResponses, projectId) => {
    ...
    const formsInProject = typeof(Forms) != 'object' ? (await Forms.find({ projectId }).lean()) : [];
