...
const resetProject = async (projectId, { projectLanguages, fallbackLang }) => {
    try {
        ...
        // await ClaiService.remove({ projectId });
        // await Models.remove({ projectid: projectId });
        ...
        // await createClaiService({ _id: projectId });
        // await createModels({ _id: projectId });
...
export const handleImportClaiService = async (files, { supportedEnvs, projectId }) => {
    if (!files.length) return [];
    const importedEnv = new Set();
    const toImport = files.filter((f) => {
        const { env } = f;
    ...
    const importResult = await Promise.all(
        toImport.map(async (f) => {
            try {
                const { env, rawText } = f;
                if (supportedEnvs.includes(env)) {
                    await Meteor.callWithPromise('claiservice.save', {
                        environment: env,
                        projectId,
                        claiservice: rawText,
                    });
...
export const handleImportModels = async (files, { supportedEnvs, projectId }) => {
    if (!files.length) return [];
    const importedEnv = new Set();
    const toImport = files.filter((f) => {
        const { env } = f;
    ...
    const importResult = await Promise.all(
        toImport.map(async (f) => {
            try {
                const { env, rawText } = f;
                if (supportedEnvs.includes(env)) {
                    await Meteor.callWithPromise('models.save', {
                        environment: env,
                        projectId,
                        models: rawText,
                    });
...
export const handleImportAll = async (files, params) => {
    ...
    try {
        ...
        importers.push(
            handleImportClaiService(
                toImport.filter(f => f.dataType === 'claiservice'),
                params,
            ),
        );
        importers.push(
            handleImportModels(
                toImport.filter(f => f.dataType === 'models'),
                params,
            ),
        );
