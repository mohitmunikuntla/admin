...
import { ClaiService, createClaiService } from '../claiservice';
    ...
    Meteor.methods({
        async 'project.insert'(item, bypassWithCI) {
            ...
            try {
                ...
                createClaiService({ _id, ...item });
                // createModels({ _id, ...item });
        ...
        'project.productionUpdates'(projectId) {
            checkIfCan(['projects:w'], projectId);
            check(projectId, String);
            try {
                const project = Projects.findOne({_id: projectId});
                if (!project) throw new Meteor.Error('Project not found');
                const service = ClaiService.findOne({$and: [{projectId}, {environment: 'production'}]});
        ...
        'project.deleteProd'(projectId) {
            checkIfCan(['projects:w'], projectId);
            check(projectId, String);
            try {
                ClaiService.remove({ projectId, environment: 'production' })
                Models.remove({ projectid: projectId, environment: 'production' })
            } catch(e) {
                throw formatError(e);
            }
        },
        ...
        async 'project.delete'(
            ...
            try {
                ...
                ClaiService.remove({ projectId }); // Delete claiservice
                Models.remove({ projectid: projectId }); // Delete Models
        ...
        async 'project.getChatProps'(projectId, environment = 'development') {
            ...
            let service = ClaiService.findOne({
                $or: [
                    { projectId, environment: { $exists: false } },
                    { projectId, environment: 'development' },
                ],
            }).fetch() || [];
            var credentials = claiservice.includes(service) ? claiservice[service.claiservice]['credentials'] : [];
