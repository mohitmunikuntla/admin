...
export const can = (permission, projectId, userId, options = {}) => {
    ...
    var projectfield = projectId ? Projects.find({_id: projectId}, { fields: { disabled: 1 } }).fetch() : [];

    if (projectfield != undefined && projectfield.length != 0){
        if (typeof(permission) == 'string' && projectfield[0].disabled){
            if (permission.split("").pop() != 'r' && permission != 'global-admin'){
                return false
            } else if (permission.split("").pop() == 'r' && permission != 'global-admin'){
                return true
            }
        }
    }
...
if (Meteor.isServer) {
    export const setUpRoles = () => {
        ...
        createRole('services:r', 'Can read bot services.');
        createRole('services:w', 'Can create, delete and edit bot services. Extends services:r.');
        Roles.addRolesToParent('services:r', 'services:w');

        createRole('models:r', 'Can read bot models.');
        createRole('models:w', 'Can create, delete and edit bot models. Extends models:r.');
        Roles.addRolesToParent('models:r', 'models:w');
        ...
        Roles.addRolesToParent(['roles:r'], 'users:r', 'services:r', 'models:r');
        createRole('users:w', 'Can add, edit, or remove user details and roles. Extends `users:r`');
        Roles.addRolesToParent(['users:r'], 'users:w', 'services:r', 'models:r');

        createRole('project-admin', 'Can access and edit all resources of a project. Extends `projects:w`, `users:w`  ');
        Roles.addRolesToParent(['projects:w', 'users:w', 'services:r', 'models:r'], 'project-admin');
        ...
        Roles.addRolesToParent(['project-admin', 'roles:w', 'global-settings:w', 'resources:w', 'services:r', 'models:r'], 'global-admin');
    };
    ...
    export const publishRoles = (context) => { // exported for testing
        if (context.userId) {
            if (can(['roles:r', 'users:r', 'services:r', 'models:r'], { anyScope: true })) {
