...
if (Meteor.isServer) {
    ...
    Meteor.methods({
        async 'rasa.parse'(instance, examples, projectId, options = {}) {
        ...
        async 'rasa.parseAPI'(instance, examples, options = {}) {
            checkIfCan('nlu-data:r', instance.projectId);
            check(instance, Object);
            check(examples, Array);
            check(options, Object);
            ...
            appMethodLogger.debug('Parsing nlu');
            try {
                const client = await createAxiosForRasaAPI(instance.projectId, { timeout: 100 * 1000 });
                addLoggingInterceptors(client, appMethodLogger);
                // axiosRetry(client, { retries: 3, retryDelay: axiosRetry.exponentialDelay });
                const requests = examples.map(({ text, lang }) => {
                    const payload = Object.assign({}, { text, lang });
        ...
        async 'rasa.train'(projectId, env = 'development') {
            ...
            try {
                ...
                if (trainingResponse.status === 200) {
                    ...
                    globalSettings?.settings?.private?.webhooks?.postTraining?.url ? Meteor.call('call.postTraining', projectId, trainingResponse.data) 
                    : Meteor.call('call.uploadModel', projectId, trainingResponse.data);
                    Activity.update(
