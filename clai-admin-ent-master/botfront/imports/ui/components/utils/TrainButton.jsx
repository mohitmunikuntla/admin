...
class TrainButton extends React.Component {
    ...
    renderDropdownMenu = () => {
        ...
        const { modalOpen } = this.state;
        const deployOptions = environments.map(env => ({
                key: env,
                value: env,
                text: `Deploy to ${env}`,
        }));
        ...
        return (
            ...
            >
                <Dropdown.Menu>
                    ...
                    {webhook && webhook.url ? deployOptions.map(opt => (
                        ...
                        <React.Fragment key={opt.key}>
                            ...
                            <Confirm
                                ...
                                onConfirm={(e) => {
                                    this.trainAndDeploy2Prod(opt.value);
                                    this.showModal(opt.value, false);
                                    e.stopPropagation();
                                }}
                                content={`Do you really want to deploy your project to ${opt.value}`}
    ...
    deploy2Prod = (target) => {
        ...
        try {
            Meteor.call('call.deployModel', projectId, target, (err, response) => {
                if (err || response === undefined || response.status !== 200) {
                    Alert.error(`Deployment failed: ${err.message}`, {
                        position: 'top-right',
                        timeout: 120000,
    ...
    trainAndDeploy2Prod = async (target) => {
        try {
            this.deploy2Prod(target);
        } catch (e) {
            Alert.error('Cannot deploy, training failed', {
                position: 'top-right',
                timeout: 3000,
