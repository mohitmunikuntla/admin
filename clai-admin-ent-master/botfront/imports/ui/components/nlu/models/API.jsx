...
export default class API extends React.Component {
    ...
    getCodeString() {
        ...
        const { query, reftime, tz, load, instance } = this.state;
        const { model } = this.props;
        load ? Meteor.call('claiservice.nlu_API', model.projectId, wrapMeteorCallback((err, data) => !err && data!={} ? this.setState({instance: data, load: false}) : null)) : null;
        const { host, token } = instance;
        if(host == undefined) {
            return ``;
        };
    ...
    parseNlu = () => {
        ...
        return Meteor.call(
            'rasa.parseAPI',
            {...instance, projectId: model.projectId},
